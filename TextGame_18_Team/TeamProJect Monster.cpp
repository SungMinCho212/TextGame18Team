...
public:
	Monster(int Health, int AttackPower, vector<string> DropItems) : Hp(Health), AttackPower(AttackPower), Items(DropItems) {}     //몬스터 생성 시 체력 값을 바로 Hp멤버에 넣음. 상속 클래스에서 자동 호출 방지.   {}은 생성자 본문. 비어있는 이유: 멤버 초기화 리스트에서 Hp를 초기화 했기 때문에 작업 필요X 
	virtual void Attack() 
	{
	...
	virtual void TakeDamge(int Damage) 
	{
		Hp -= Damage;
		if (0 > Hp) Hp = 0;
		cout << "몬스터가 " << Damage << "의 피해를 입었다!!! (남은 체력: " << Hp << ")" << endl;
	}
};
...
{
public:
	Goblin() : Monster(100, 10, { "고블린 아이템1", "고블린 아이템2" }) 
	{
...
{
public:
	Orc() : Monster(100, 10, {"오크 아이템1", "오크 아이템2"}) {
...
{
public:
	Troll() : Monster(100, 10, { "트롤 아이템1", "트롤 아이템2" }) {
...
{
public:
	Slime() : Monster(100, 10, { "슬라임 아이템1", "슬라임 아이템2" }) {
